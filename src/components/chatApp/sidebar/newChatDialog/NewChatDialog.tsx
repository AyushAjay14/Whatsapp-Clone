...
function NewChatDialog({ setShowDialog }: { setShowDialog: (showDialog: boolean) => void }) {
  ...
  function handleAddNewConnection() {
    ...
    const newConnectionArray = [...connections, { id, name: newName, profileImg: PORFILE_IMG }];
  ...
  return (
    <div className="new-chat-dialog__container">
      ...
      <div className="new-chat-dialog__btn__container">
        <button className="new-chat-dialog__btn" onClick={() => setShowDialog(false)}>
          Cancel
        </button>
        <button className="new-chat-dialog__btn" onClick={handleAddNewConnection}>
          Start new chat
        </button>
      </div>
    </div>
